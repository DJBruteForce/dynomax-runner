...
$logRoot = Join-Path $dynomaxRoot 'Logs\Patches\1.0.10-S2'
[System.IO.Directory]::CreateDirectory($logRoot) | Out-Null
$logPath = Join-Path $logRoot ('Dynomax-Patch-1.0.10-S2-' + [DateTime]::UtcNow.ToString('yyyyMMddHHmmss') + '.log')
...
try {
    ...
    if ([string]$manifest.patchVersion -ne '1.0.10-S2') { throw 'Patch manifest version is not 1.0.10-S2.' }
    ...
    Write-Host 'DYNOMAX CORE 1.0.10 SECRET BRIDGE AND OUTPUT REDACTION HOTFIX S2' -ForegroundColor Cyan
    Write-Host '============================================================' -ForegroundColor Cyan
    ...
    if ($alreadyInstalled) {
        Write-Host 'Dynomax Core 1.0.10 secret/output bridge S2 is already installed with the exact packaged bytes.' -ForegroundColor Green
    } else {
    ...
    Write-Host ''
    Write-Host 'Dynomax Core 1.0.10 secret/output bridge S2 installed successfully.' -ForegroundColor Green
    ...
    Write-Host ''
    Write-Host 'DYNOMAX CORE 1.0.10 SECRET AND OUTPUT S2 PATCH FAILED' -ForegroundColor Red
