...
## Failure console and diagnostic rule

Every user-launched BAT entry point must:

1. Close automatically after a successful run.
2. Keep the console open with `pause` after a non-zero exit code.
3. Print the exit code and the diagnostic locations.
4. Never hide or immediately close a failed PowerShell, Robot, Playwright, installer or workflow execution.
5. Write patch/install transcripts under `C:\Dynomax\Logs` when the failure can occur before a result ZIP is generated.
6. Preserve a failed workflow result ZIP under `C:\Dynomax\Exports` whenever the workflow reached result packaging.

Project installers supplied by an agent must inherit this rule from the current Action and Workflow BAT templates.


## Windows PowerShell executable-source rule

All executable `.ps1`, `.bat` and `.cmd` files delivered by an agent must be ASCII-only. Decorative Unicode such as smart quotes, em dashes, arrows and ellipses is prohibited in executable files. Before a package may copy or run a PowerShell payload, Windows PowerShell 5.1 must parse every delivered `.ps1` file through `System.Management.Automation.Language.Parser.ParseFile`. The same parser gate must run again against the active installed Core, Config-And-Setup and Project-Setup scripts after copying. A parser error or non-ASCII executable is `TEST_INVALID` and must stop before a workflow starts. Markdown, JSON and other non-executable reporting files may use UTF-8 where appropriate.
