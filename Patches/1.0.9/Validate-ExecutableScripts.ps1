...
if (-not (Test-Path -LiteralPath $resolvedRoot)) {
    throw "Parser-gate root was not found: '$resolvedRoot'."
}
...
if ($ManifestPath) {
    ...
    foreach ($entry in @($manifest.files)) {
        ...
        if (-not (Test-Path -LiteralPath $candidate -PathType Leaf)) {
            throw "Manifest executable was not found: '$candidate'."
        }
        $item = Get-Item -LiteralPath $candidate
        if ($item.Extension -in @('.ps1', '.bat', '.cmd', '.robot', '.resource')) {
            $files += $item
        }
    }
} elseif (Test-Path -LiteralPath $resolvedRoot -PathType Leaf) {
    $files = @(Get-Item -LiteralPath $resolvedRoot)
} else {
    $files = @(Get-ChildItem -LiteralPath $resolvedRoot -Recurse -File | Where-Object {
        $_.Extension -in @('.ps1', '.bat', '.cmd', '.robot', '.resource')
    })
}
...
foreach ($file in @($files | Sort-Object FullName -Unique)) {
    ...
    if ($nonAsciiIndex -ge 0) {
        Write-Host ("[ParserGate] NON-ASCII executable: {0}; offset {1}; value 0x{2:X2}" -f $file.FullName, $nonAsciiIndex, $bytes[$nonAsciiIndex]) -ForegroundColor Red
        $failureCount++
        continue
    }
    ...
    if ($file.Extension -ieq '.ps1') {
        ...
        foreach ($parseError in @($parseErrors)) {
            Write-Host ("[ParserGate] PARSE ERROR: {0}:{1}:{2}: {3}" -f $file.FullName, $parseError.Extent.StartLineNumber, $parseError.Extent.StartColumnNumber, $parseError.Message) -ForegroundColor Red
            $failureCount++
        }
    }
}
