...
$logPath = Join-Path $logRoot ('Dynomax-Patch-1.0.9-' + [DateTime]::UtcNow.ToString('yyyyMMddHHmmss') + '.log')
...
try {
    ...
    if ([string]$manifest.patchVersion -ne '1.0.9') { throw 'Patch manifest version is not 1.0.9.' }
    ...
    if ($currentVersion -lt [version]'1.0.8' -or $currentVersion -gt [version]'1.0.9') {
        throw "Dynomax 1.0.8 or 1.0.9 is required. Current version: $currentVersionText"
    }

    $windowsPowerShell = (Get-Command 'powershell.exe' -ErrorAction Stop).Source
    Write-Host ''
    Write-Host '============================================================' -ForegroundColor Cyan
    Write-Host 'DYNOMAX CORE 1.0.9 EXACT-VERSION COMPATIBILITY PATCH' -ForegroundColor Cyan
    Write-Host '============================================================' -ForegroundColor Cyan
    ...
    Invoke-DynomaxNativeStep -Name 'Payload exact-version contract checks' -Executable $windowsPowerShell -Arguments @('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',$contractPath,'-SourceRoot',$payloadRoot,'-AssertNoSqlFiles')
    ...
    if ($alreadyInstalled) {
        Write-Host 'Dynomax Core 1.0.9 is already installed with the exact packaged bytes.' -ForegroundColor Green
    } else {
    ...
    Invoke-DynomaxNativeStep -Name 'Installed exact-version contract checks' -Executable $windowsPowerShell -Arguments @('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',$contractPath,'-SourceRoot',$dynomaxRoot)
    ...
    Write-Host ''
    Write-Host 'Dynomax Core 1.0.9 exact-version patch installed successfully.' -ForegroundColor Green
    ...
    Write-Host ''
    Write-Host 'DYNOMAX CORE 1.0.9 PATCH FAILED' -ForegroundColor Red
