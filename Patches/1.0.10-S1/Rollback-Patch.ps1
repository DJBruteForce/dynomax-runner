...
$logPath = Join-Path $logRoot ('Dynomax-Rollback-1.0.10-S1-' + [DateTime]::UtcNow.ToString('yyyyMMddHHmmss') + '.log')
...
try {
    ...
    if (-not $BackupDirectory) {
        ...
        if (-not $latest) { throw 'No Dynomax 1.0.10 secret bridge S1 backup directory was found.' }
    ...
    if ([string]$manifest.patchVersion -ne '1.0.10-S1') { throw 'The selected backup is not a Dynomax 1.0.10 secret bridge S1 backup.' }
    ...
    Write-Host 'Dynomax 1.0.10 secret bridge S1 rollback completed. No SQL rollback was required.' -ForegroundColor Green
    $exitCode = 0
} catch {
    Write-Host 'DYNOMAX 1.0.10 SECRET BRIDGE S1 ROLLBACK FAILED' -ForegroundColor Red
