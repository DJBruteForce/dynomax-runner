...
$logRoot = Join-Path $dynomaxRoot 'Logs\Patches\1.0.10'
[System.IO.Directory]::CreateDirectory($logRoot) | Out-Null
$logPath = Join-Path $logRoot ('Dynomax-Patch-1.0.10-' + [DateTime]::UtcNow.ToString('yyyyMMddHHmmss') + '.log')
...
try {
    ...
    if ([string]$manifest.patchVersion -ne '1.0.10') { throw 'Patch manifest version is not 1.0.10.' }
    ...
    if ($currentVersion -lt [version]'1.0.9' -or $currentVersion -gt [version]'1.0.10') {
        throw "Dynomax 1.0.9 or 1.0.10 is required. Current version: $currentVersionText"
    }

    $windowsPowerShell = (Get-Command 'powershell.exe' -ErrorAction Stop).Source
    Write-Host ''
    Write-Host '============================================================' -ForegroundColor Cyan
    Write-Host 'DYNOMAX CORE 1.0.10 DATABASE RUNTIME BRIDGE PATCH' -ForegroundColor Cyan
    Write-Host '============================================================' -ForegroundColor Cyan
    ...
    Invoke-DynomaxNativeStep -Name 'Payload database-runtime bridge contract checks' -Executable $windowsPowerShell -Arguments @('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',$contractPath,'-SourceRoot',$payloadRoot,'-AssertNoSqlFiles')
    ...
    if ($alreadyInstalled) {
        Write-Host 'Dynomax Core 1.0.10 is already installed with the exact packaged bytes.' -ForegroundColor Green
    } else {
    ...
    Invoke-DynomaxNativeStep -Name 'Installed database-runtime bridge contract checks' -Executable $windowsPowerShell -Arguments @('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',$contractPath,'-SourceRoot',$dynomaxRoot)
    ...
    Write-Host ''
    Write-Host 'Dynomax Core 1.0.10 database runtime bridge installed successfully.' -ForegroundColor Green
    ...
    Write-Host ''
    Write-Host 'DYNOMAX CORE 1.0.10 PATCH FAILED' -ForegroundColor Red
