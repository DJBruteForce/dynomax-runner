...
def test_runtime_contract_identity_and_hash_closure():
    ...
    assert "user-interaction-response-materialization-v1" in manifest["capabilities"]
...
def test_response_payload_is_materialized_before_same_run_resume():
    control = read("Core/Execution/Dynomax.ControlFlow.ps1")
    invoke = read("Core/Invoke-DynomaxWorkflow.ps1")
    assert "function Set-DynomaxInteractionResponseOutputs" in control
    assert "responseSchema" in control
    assert "ResponseValues" in control
    assert "Set-DynomaxInteractionResponseOutputs" in invoke
    assert "ProtectedResponseValues" not in invoke
    assert "InteractionCheckpointResumed" in control
