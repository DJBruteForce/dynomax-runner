...
function Invoke-DynomaxPendingParallelFork {
    ...
    $timing=[ordered]@{schemaVersion=1;forkNodeId=$forkId;maximumParallelism=$max;startedAtUtc=[string]$fork.startedAtUtc;endedAtUtc=[DateTime]::UtcNow.ToString('o');branches=@($orderedExecutions|Sort-Object Index|ForEach-Object{$firstStep=@($_.Steps|Select-Object -First 1);[ordered]@{index=[int]$_.Index;firstActionNodeId=$(if($firstStep.Count){[string](Get-DynomaxPropertyValue -Object $firstStep[0] -Name 'workflowNodeId' -DefaultValue '')}else{''});firstActionKey=$(if($firstStep.Count){[string](Get-DynomaxPropertyValue -Object $firstStep[0] -Name 'actionId' -DefaultValue '')}else{''});status=[string]$_.Result.status;startedAtUtc=$_.Result.startedAtUtc;endedAtUtc=$_.Result.endedAtUtc}})}
