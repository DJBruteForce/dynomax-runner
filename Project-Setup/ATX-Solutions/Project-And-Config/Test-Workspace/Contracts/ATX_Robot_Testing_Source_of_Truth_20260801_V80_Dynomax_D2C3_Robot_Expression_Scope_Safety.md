# ATX Robot Testing Source of Truth V79
...
60  atx.banking.d2c3.guided-banking-acceptance          v5
70  atx.banking.d2c3.manager-data-management-acceptance v5
...
## V79 Windows-path-safe Robot expression contract

Live V1.0.16 evidence proved that Payment Evidence, reusable Structured-profile preparation, and
independent cleanup all passed. Guided Banking failed before its first page action because the
prerequisite condition interpolated a Windows file path into a quoted Python-style Robot expression:

```robot
IF    '${profile}' == '' or '${sample}' == '' or '${sample_name}' == ''
```

A value such as `C:\Users\...` makes Python parse `\U` as a Unicode escape. Manager Data
Management contained the same latent defect and would have failed next.

Path-bearing and other arbitrary strings must be evaluated using Robot object-variable syntax:
...
- Do not interpolate `${sample}` or another Windows path inside a quoted `IF` expression.
- Use `$variable` object-variable syntax for string comparisons in Robot expressions.
- The installer must certify the safe expression and reject the former quoted expression in both
  Guided Banking and Manager Data Management resources before workflow import.
- Guided Banking is immutable action v5.
- Manager Data Management is immutable action v5.
- V1.0.16 workflow hash remains compatible for safe SQL-backed session reuse.

This is a test-package correction only. It changes no ATX application source, SQL, production data,
ownership rule, selector, or browser workflow after the prerequisite gate.

## V80 Robot object-variable nested-scope safety contract

Live V1.0.17 evidence proved that Payment Evidence, reusable Structured-profile preparation, and
independent cleanup passed. Guided Banking failed after session creation because the schema expression
used a Robot object variable inside a Python generator expression:

```robot
all(key in $session for key in ['id', 'step', ...])
```

Python evaluates the generator body in a nested scope where Robot's `$session` object variable is not
visible. The same latent defect existed in active-session validation and shared Guided-session cleanup.

The certified form performs no comprehension or generator over a Robot object variable:

```robot
set(['id', 'step', ...]).issubset(set($session.keys()))
```

Permanent rules:

- A Robot `$variable` must not be referenced from the body of a Python generator, list comprehension,
  set comprehension, dictionary comprehension, or lambda used by `Evaluate`.
- Session schema checks must use a non-nested set/subset expression or ordinary Robot keywords.
- The installer must inspect packaged and installed Guided/common resources and reject
  `all(key in $active for key in ...)` and `all(key in $session for key in ...)`.
- Guided Banking is immutable action v6.
- Cleanup is immutable action v10 because its shared Guided-session cleanup behavior changed.
- Manager Data Management remains immutable action v5.
- V1.0.17 workflow hash remains compatible for safe session reuse.

This is a test-package correction only. It changes no ATX application source, SQL, selector,
production data, ownership rule, or accepted browser workflow.
