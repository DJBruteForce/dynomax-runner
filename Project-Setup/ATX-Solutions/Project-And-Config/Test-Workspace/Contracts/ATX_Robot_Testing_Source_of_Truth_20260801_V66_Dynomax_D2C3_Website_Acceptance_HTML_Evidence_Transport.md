# ATX Robot Testing Source of Truth V66 - Dynomax D2-C3 Website Acceptance with HTML Evidence Transport

## Superseded contract

V65 is retired. V65 corrected the scenario dependency order and evidence interpretation, but its HTML capture callback contained a Robot-recognized `\n` escape inside a JavaScript string literal. Robot transported that escape as a physical newline, so Browser received invalid JavaScript and both the first functional capture and the first cleanup capture failed before evidence could be written.

V64 is also retired. Its shared Structured-profile fixture step ran before Payment Evidence, so a missing Structured profile incorrectly skipped a scenario that did not depend on that profile. V64 also allowed filtered profile results to be described as total workspace inventory without captured page HTML.
...
## Robot-to-Browser JavaScript transport rules

- JavaScript passed inline through Robot `Evaluate JavaScript` must not contain Robot-recognized `\n`, `\r` or `\t` escape sequences. Robot can convert them before Browser evaluates the callback, changing valid source text into invalid JavaScript.
- Complete HTML serialization must use `'<!DOCTYPE html>'+clone.outerHTML` or another source-certified expression that contains no Robot-recognized transport escape.
- Package and installed-resource preflight must scan every inline `Evaluate JavaScript` line and reject `\n`, `\r` and `\t` before the action-block dry run.
- A dry-run PASS proves Robot keyword resolution only; it does not prove that an inline JavaScript callback can execute against a live page. The transport scan is therefore mandatory.
- The first HTML capture in both the normal block and cleanup block must execute before any destructive cleanup action. A capture transport failure is `TEST_INVALID`, not an ATX application failure.
...
- Normal and cleanup action blocks pass the inline-JavaScript transport preflight before live execution.

## Classification

- `PASS`: all three website paths and cleanup pass.
- `FAIL`: ATX violates a current source-certified website contract.
- `TEST_INVALID`: selector, dependency chain, cardinality, page interpretation, expectation, or Robot-to-Browser JavaScript transport is stale/incorrect.
