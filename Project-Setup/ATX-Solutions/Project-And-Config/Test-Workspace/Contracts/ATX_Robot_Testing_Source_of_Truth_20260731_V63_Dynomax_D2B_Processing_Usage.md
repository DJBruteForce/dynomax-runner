...
## V63 D2-B active-sample continuity correction addendum

The V1.0.6 live run made measurable forward progress: direct selectable-PDF processing, sample clearing and image preparation all passed. The workflow reached image OCR for the first time. OCR then rendered `Mode: Unknown`, zero pages and the visible message asking the user to choose a sample. Cleanup passed and deleted the exact disposable Draft.

The causal classification is `TEST_INVALID`, not an ATX OCR failure. The shared `ATX D2B Ensure Exact Profile Open` helper performed an unconditional full navigation at the start of every action. That navigation discarded the browser-local file selection and the hidden `SampleCacheToken` established by the preceding Prepare Preview AJAX action. The OCR action therefore submitted without an active sample.

V63 requires:

- Preserve the current Statement page when `#Form_Id` already matches the exact context `statementProfileId`.
- Navigate to `statementProfileUrl` only when the Statement page is absent or belongs to another profile.
- Re-run Statement-page initialization checks without reloading when the exact page is already active.
- Before OCR, cached Remap and bounded recovery, require either a selected file in `#SampleEvidence` or a non-empty `#statementProfileForm input[name="SampleCacheToken"]`.
- Classify a missing active sample at an action boundary as `TEST_INVALID`; do not report it as an ATX OCR failure.
- Preserve the current session `000003-Phase25H-D2B-Processing-Usage`.

No ATX application, ATX database or Dynomax Core change is required.
