...
foreach ($candidate in @(Get-ChildItem -LiteralPath $exportRoot -Filter '*.zip' -File | Where-Object { $_.LastWriteTimeUtc -ge $startedAtUtc.AddSeconds(-5) } | Sort-Object LastWriteTimeUtc -Descending)) {
    try {
        if (Test-AtxDynomaxResultZip -Path $candidate.FullName -ProjectKey 'atx-solutions' -WorkflowId 'atx.phase25h.d2c3.website-acceptance') {
            $result = $candidate
            break
        }
    }
...
if ($null -eq $result) { throw 'No newly generated valid ATX D2-C3 Dynomax result ZIP was found under C:\Dynomax\Exports.' }
