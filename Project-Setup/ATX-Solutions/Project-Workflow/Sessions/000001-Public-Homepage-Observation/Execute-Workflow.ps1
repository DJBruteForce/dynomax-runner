...
foreach ($candidate in @(Get-ChildItem -LiteralPath $exportRoot -Filter '*.zip' -File | Where-Object { $_.LastWriteTimeUtc -ge $startedAtUtc.AddSeconds(-5) } | Sort-Object LastWriteTimeUtc -Descending)) {
    try {
        if (Test-AtxDynomaxResultZip -Path $candidate.FullName -ProjectKey 'atx-solutions' -WorkflowId 'atx.initial.public-homepage-observation') {
            $result = $candidate
            break
        }
    }
...
if ($null -eq $result) { throw 'No newly generated, valid ATX public-homepage Dynomax result ZIP was found under C:\Dynomax\Exports.' }
