...
public sealed partial class IndexModel : PageModel
{
    ...
    public string? OpenDialogId { get; private set; }
    ...
    [BindProperty] public VersionUploadInput VersionInput { get; set; } = new();
    ...
    public async Task<IActionResult> OnPostUploadVersionAsync(Guid projectId, CancellationToken cancellationToken)
    {
        ProjectContextRequestAccess? access = await LoadAccessAsync(projectId, cancellationToken);
        if (access is null || !access.CanUpload) return Forbid();
        KeepModelStatePrefix("VersionInput");
        if (VersionInput.ResourceId == Guid.Empty) ModelState.AddModelError("VersionInput.ResourceId", "Select a Project Context file to update.");
        if (VersionInput.ExpectedCurrentVersionId == Guid.Empty) ModelState.AddModelError("VersionInput.ExpectedCurrentVersionId", "The Current version identity is missing. Refresh Project Context and try again.");
        if (VersionInput.File is null || VersionInput.File.Length <= 0) ModelState.AddModelError("VersionInput.File", "Select one replacement file.");
        if (VersionInput.File?.Length > ProjectContextLimits.MaxUploadBytes) ModelState.AddModelError("VersionInput.File", $"The replacement file exceeds the {ProjectContextLimits.MaxUploadBytes / (1024 * 1024)} MiB upload limit.");

        ProjectContextResourceDetails? details = VersionInput.ResourceId == Guid.Empty ? null : await _context.GetResourceDetailsAsync(access, VersionInput.ResourceId, cancellationToken);
        if (VersionInput.ResourceId != Guid.Empty && details is null) ModelState.AddModelError("VersionInput.ResourceId", "The Project Context file was not found.");
        if (!ModelState.IsValid)
        {
            OpenDialogId = "pc-version-dialog";
            await LoadResourcesAsync(access, null, details?.Resource.Category, null, details?.Resource.ResourceId, false, null, null, 1, 20, cancellationToken);
            return Page();
        }

        DateTime? clientLastModifiedUtc = null;
        if (!string.IsNullOrWhiteSpace(VersionInput.ClientLastModifiedUtc) && DateTimeOffset.TryParse(VersionInput.ClientLastModifiedUtc, out DateTimeOffset clientModified))
            clientLastModifiedUtc = clientModified.UtcDateTime;
        try
        {
            IFormFile file = VersionInput.File!;
            await using var stream = new MemoryStream(checked((int)file.Length));
            await file.CopyToAsync(stream, cancellationToken);
            ProjectContextResourceSummary current = details!.Resource;
            UploadProjectContextFileResult result = await _context.UploadAsync(access, new UploadProjectContextFileCommand(
                projectId, file.FileName, file.ContentType, stream.ToArray(), current.LogicalPath ?? current.LogicalKey, current.Category, current.Description, current.Tags,
                true, current.AgentReadable == true, current.IsAuthoritative, access.ActorUserId, DateTime.UtcNow, false,
                current.ResourceId, VersionInput.ExpectedCurrentVersionId, clientLastModifiedUtc, VersionInput.AllowOlderFile), cancellationToken);
            TempData["Success"] = result.ResourceVersionId == VersionInput.ExpectedCurrentVersionId
                ? "The selected file is identical to the existing Current version; no duplicate version was created."
                : $"Current version is now v{result.VersionNumber}. Previous versions remain available in history subject to the Project Context retention policy.";
            return RedirectToPage(new { projectId, category = current.Category, resourceId = result.ResourceId });
        }
        ...
        {
            ModelState.AddModelError("VersionInput.File", exception.Message);
            OpenDialogId = "pc-version-dialog";
            await LoadResourcesAsync(access, null, details?.Resource.Category, null, details?.Resource.ResourceId, false, null, null, 1, 20, cancellationToken);
            return Page();
        }
    ...
    public async Task<IActionResult> OnPostImportFolderAsync(Guid projectId, CancellationToken cancellationToken)
    {
        ...
        KeepModelStatePrefix("FolderInput");
    ...
    {
        string start = prefix + ".";
        foreach (string key in ModelState.Keys.Where(key => !key.Equals(prefix, StringComparison.Ordinal) && !key.StartsWith(start, StringComparison.Ordinal)).ToArray())
    ...
    public sealed class VersionUploadInput
    {
        public Guid ResourceId { get; set; }
        public Guid ExpectedCurrentVersionId { get; set; }
        public IFormFile? File { get; set; }
        [StringLength(100)] public string? ClientLastModifiedUtc { get; set; }
        public bool AllowOlderFile { get; set; }
    }
